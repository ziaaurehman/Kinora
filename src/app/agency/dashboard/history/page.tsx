...
export default function InquiryHistoryPage() {
  const history = [
    ...
    { id: 5, name: 'Courtney Henry', zip: '84600', care: 'Dementia Care', date: '9-02-24', status: 'Accepted' },
  ];
  ...
  return (
    <div className="w-full max-w-[1200px] mx-auto p-6 md:p-8 font-sans bg-[#FAF9F6] min-h-screen">
      ...
      <div className="flex items-start gap-4 mb-8">
        ...
        <div>
          <h1 className="text-[28px] font-bold text-[#112330] mb-1 tracking-tight leading-none mt-1">Inquiry History</h1>
          <p className="text-[15px] text-[#64748B]">Review all past inquiries with statuses and timelines.</p>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead>
              <tr className="text-[14px] font-semibold text-[#64748B]">
                <th className="pb-4 pr-4 border-b border-gray-200">Family Name</th>
            ...
            <tbody className="text-[14px] font-semibold text-[#112330]">
              {history.map((item) => (
                <tr key={`hist-${item.id}`} className="hover:bg-gray-50 transition-colors">
                  <td className="py-5 pr-4 border-b border-gray-100">{item.name}</td>
                  ...
                  <td className="py-5 pr-4 text-gray-500 border-b border-gray-100 font-medium">{item.date}</td>
                  <td className="py-5 border-b border-gray-100">
                    {item.status === 'Accepted' && (
                      <span className="inline-flex items-center px-3 py-1.5 rounded-md text-[13px] font-bold bg-[#DCFCE7] text-[#16A34A]">Accepted</span>
                    )}
                    {item.status === 'Expired' && (
                      <span className="inline-flex items-center px-3 py-1.5 rounded-md text-[13px] font-bold bg-gray-100 text-gray-500">Expired</span>
                    )}
                    {item.status === 'Declined' && (
                      <span className="inline-flex items-center px-3 py-1.5 rounded-md text-[13px] font-bold bg-[#FEE2E2] text-[#EF4444]">Declined</span>
                    )}
                  </td>
                </tr>
              ))}
